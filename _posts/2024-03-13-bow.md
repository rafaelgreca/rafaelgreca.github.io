...
In text classification, **the Bag of Words (BoW) method records the number of occurrences of each bag that is created for each instance type or word disregarding the order of the words or the grammar**. And in visual scene classification **it is based on clusters of local descriptors which are taken from the images disregarding the order of the clusters**. [1]

## Notes

In BoW technique, **the images is treated same as documents**, and the **features in the images are the same as the words in the documents**. [1]
...
**The number of occurrences of a word or a term in a text document is known as term frequency**, which will be used to specify the category of the text or simply classifying the text, and it's also called **"histogram representation"** of the document. [1]
